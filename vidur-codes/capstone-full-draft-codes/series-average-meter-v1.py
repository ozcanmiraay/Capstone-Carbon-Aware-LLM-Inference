...
class SeriesAverageMeter:
    ...
    def put(self, data_x: float, data_y: float) -> None:
        ...
        # Skip appending unnecessary zero values unless it is meaningful
        if data_y == 0 and (self._last_data_y == 0 or not self._distribution):
            logger.debug(f"Skipping unnecessary zero MFU value for time={data_x}")
            return

        # Add data to the distribution list
        self._distribution.append({"time": data_x, "mfu": data_y})
        logger.debug(f"Appended MFU value: time={data_x}, mfu={data_y}")
    ...
    def print_stats(self, name: str, path: str) -> None:
            ...
            stats_dict = {
                ...
                f"{name}_distribution": self._distribution  # Include the full distribution
            }
