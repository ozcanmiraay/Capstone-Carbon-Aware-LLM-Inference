...
class BatchStage(BaseEntity):
    def __init__(
        ...
        logger.debug(f"BatchStage initialized with batch_id={self._batch_id}, id={self._id}")
        assert self._batch_id is not None, "Batch ID must be initialized!"
        assert self._id is not None, "Batch Stage ID must be initialized!"
