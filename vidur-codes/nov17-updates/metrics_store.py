...
class MetricsStore:
    ...
    def on_replica_stage_schedule(
        ...
        # Ensure BatchStage IDs are valid
        logger.debug(f"on_replica_stage_schedule called with time={time}, replica_id={replica_id}, stage_id={stage_id}, batch_stage={batch_stage}")
        if batch_stage._batch_id is None or batch_stage._id is None:
            logger.warning(f"BatchStage {batch_stage._id} has missing IDs. Skipping MFU logging.")
            return

        # Proceed with MFU calculation
        mfu = self._mfu_calculator.get_mfu(batch_stage)

        cumulative_time = (
            self._mfu_distribution[-1]["time"] + batch_stage.execution_time
            if self._mfu_distribution
            else batch_stage.execution_time
        )
        self._mfu_distribution.append({
            "time": cumulative_time,
            "mfu": mfu,
            "batch_id": batch_stage._batch_id,
            "batch_stage_id": batch_stage._id,
        })
        logger.debug(f"MFU logged for Batch ID: {batch_stage._batch_id}, Batch Stage ID: {batch_stage._id}, MFU: {mfu}, Time: {cumulative_time}")


        # Update per-replica MFU
        self._replica_mfu[replica_id - 1][stage_id - 1].put(
            time, mfu, batch_id=batch_stage._batch_id, batch_stage_id=batch_stage._id
    )
