...
def process_run(run_dir: str):
    ...
    try:
        ...
        logger.error(f"FileNotFoundError: {e} in {run_dir}")
        return None
    
    # Check if replica_scheduler_config exists under cluster_config and access the scheduler name
    if "replica_scheduler_config" in config["cluster_config"]:
        scheduler_name = config["cluster_config"]["replica_scheduler_config"].get("name", None)
        if scheduler_name == "sarathi" and config["cluster_config"]["replica_scheduler_config"].get("chunk_size", None) == 4096:
            config["cluster_config"]["replica_scheduler_config"]["name"] = "orca+"
    else:
        logger.warning("'replica_scheduler_config' not found in cluster_config")
...
def process_trace(sim_results_dir: str):
    ...
    df["num_gpus"] = (
        df["cluster_config"].apply(lambda x: x.get("num_replicas", np.nan))
        * df["cluster_config"].apply(lambda x: x.get("replica_config", {}).get("tensor_parallel_size", np.nan))
        * df["cluster_config"].apply(lambda x: x.get("replica_config", {}).get("num_pipeline_stages", np.nan))
    )
    ...
    if "poisson_request_interval_generator_qps" in df.columns:
        ...
        df["num_gpus"] * df["cluster_config"].apply(lambda x: x.get("replica_config", {}).get("device", np.nan)).map(GPU_COSTS)
    )
    ...
    df["hour_cost_per_replica"] = (
        df["cluster_config"].apply(lambda x: x.get("replica_config", {}).get("device", np.nan)).map(GPU_COSTS) * df["num_replica_gpus"]
    )

    if "poisson_request_interval_generator_qps" in df.columns:
        df["capacity_per_replica"] = (
        df["poisson_request_interval_generator_qps"] / df["cluster_config"].apply(lambda x: x.get("num_replicas", np.nan))
    )
