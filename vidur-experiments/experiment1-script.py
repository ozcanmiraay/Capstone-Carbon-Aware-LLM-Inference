...
replicas_list = [1, 4, 8, 16]
tensor_parallel_size_list = [1, 2, 4] # 8 and above tensor parallel sizes aren't supported
...
def run_simulation(replicas, tensor_parallel_size, run_number):
    ...
    stats_extraction_command = [
        "python", "vidur/config_optimizer/analyzer/stats_extractor_energy_carbon.py",
        "--sim-results-dir", renamed_dir  # Now point to the renamed "simulation-run" directory
    ]
    
    # Run the stats extraction and capture output
    result = subprocess.run(stats_extraction_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
run_number = 1
results = []
for replicas, tensor_parallel_size in product(replicas_list, tensor_parallel_size_list):
    print(f"Running simulation with {replicas} replicas and tensor parallel size {tensor_parallel_size}...")
    energy, carbon = run_simulation(replicas, tensor_parallel_size, run_number)
    results.append({
        "run": run_number,
        "replicas": replicas,
