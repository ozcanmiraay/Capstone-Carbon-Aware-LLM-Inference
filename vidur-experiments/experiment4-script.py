...
# Define the range for max tokens per request (30 points from 128 to 4096)
max_tokens_list = np.linspace(128, 4096, 30, dtype=int)  # Adjusted to use 30 points
...
for max_tokens in max_tokens_list:
    print(f"Running simulation with {max_tokens} max tokens...")
    energy, carbon, gpu_hours, mfu = run_simulation(max_tokens, run_number)
    results.append({
        "run": int(run_number),  # Ensure native Python int
        "max_tokens": int(max_tokens),  # Ensure native Python int
