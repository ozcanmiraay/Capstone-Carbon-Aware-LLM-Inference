...
def process_run(run_dir: str):
    ...
    ## ADD THE POWER MODEL HERE INSPIRED BY LLMCARBON
...
def process_trace(sim_results_dir: str):
    ...
    * df["cluster_config"].apply(lambda x: x["replica_config"]["num_pipeline_stages"])
    ...
    df["capacity_per_dollar"] =  df["request_generator_config"].apply(lambda x: x["interval_generator_config"]["qps"]) / (
    ...
    df["request_generator_config"].apply(lambda x: x["interval_generator_config"]["qps"]) / df["cluster_config"].apply(lambda x: x["num_replicas"])
    )
    ...
    avg_capacity_per_dollar = df["capacity_per_dollar"].mean()
    avg_capacity_per_replica = df["capacity_per_replica"].mean()
    ...
    simulation_stats = {
        "avg_capacity_per_dollar": avg_capacity_per_dollar,
        "avg_capacity_per_replica": avg_capacity_per_replica,
        "gpu_cost": gpu_cost,
